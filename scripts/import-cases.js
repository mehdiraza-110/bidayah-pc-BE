...
const MD_FILE = path.join(__dirname, '..', 'Cases.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'CASE');
const CATEGORY_NAME = 'Gaming Casing';
...
// Like Coolers/PSU, this sheet has no F- columns at all — just Product + Price. No web research
// was done for this one (by request): Color and Form Factor below are pulled only from text
// already present in each product's own name, nothing looked up externally. Most case specs that
// would actually matter for a builder (tower size, max GPU/cooler clearance, motherboard support
// beyond the couple of explicit Micro-ATX models) aren't in the name and are left out rather than
// guessed. A product with no color/form-factor word in its name simply gets no such feature row.
const FEATURE_KEYS = ['Color', 'Form Factor'];
...
function vendorForCase(name) {
  if (/^Be Quiet!/i.test(name)) return 'Be Quiet!';
  if (/^Lian Li/i.test(name)) return 'Lian Li';
  if (/^Fractal Design/i.test(name)) return 'FRACTAL';
  if (/^Deepcool/i.test(name)) return 'DEEPCOOL';
  return name.trim().split(/\s+/)[0].toUpperCase();
}

// "WH" is this catalog's recurring abbreviation for White (seen in Motherboards/Coolers too).
function colorFor(name) {
  if (/\bWH\b/.test(name)) return 'White';
  const m = name.match(/\b(Black|White|Silver|Gray|Grey|Snow)\b/i);
  return m ? `${m[1][0].toUpperCase()}${m[1].slice(1).toLowerCase()}` : null;
}

function formFactorFor(name) {
  if (/Micro-ATX|\bmATX\b/i.test(name)) return 'Micro-ATX';
  return null;
}
...
function parseMarkdown() {
  ...
  return rows.map((line) => {
    ...
    return {
      ...
      formFactor: formFactorFor(rawName),
    };
  });
}
...
async function main() {
  ...
  console.log('Parsing Cases data sheet...');
  const cases = parseMarkdown();
  console.log(`Parsed ${cases.length} case rows.`);
  ...
  console.log(`Matched ${matched.length}/${cases.length} cases to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / color / form factor):');
  for (const c of matched) {
    console.log(`  ${c.name} -> ${vendorForCase(c.name)} / AED ${c.price} / ${c.color || '-'} / ${c.formFactor || '-'}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((c) => vendorForCase(c.name)))];
  ...
  if (skippedExisting > 0) {
    console.log(`Skipping ${skippedExisting} products that already exist in the Gaming Casing category.`);
  }
  ...
  await runPool(toInsert, 6, async (c) => {
    ...
    const vendorId = vendorIdByName.get(vendorForCase(c.name));
