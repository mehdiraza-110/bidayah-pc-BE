...
const MD_FILE = path.join(__dirname, '..', 'New-Product-Data-Rams.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'RAM');
const CATEGORY_NAME = 'Ram';
...
// ---------- known source-data corrections ----------
// Cross-checking every row's [DDR, MHz, modules] bracket against its own F- columns caught a few
// spots where the column disagreed with the product's own name — same pattern seen in the GPU/
// CPU/Motherboard sheets, where the bracketed spec in the name is the reliable one and the loose
// column is what has the copy-paste error.
const CORRECTIONS = {
  // "DOMINATOR Titanium RGB" line — every other entry in this line is tagged RGB=YES; this one
  // was the odd one out despite "RGB" being right there in its own name.
  '32GB Corsair DOMINATOR Titanium RGB Black [DDR5, 6400MHz, 2x16GB]': { rgb: 'YES' },
  // Name says 6000MHz; the Memory Speed column had 5600MHz.
  '64GB Corsair Vengeance RGB Black [DDR5, 6000MHz, 2x32GB]': { speed: '6000MHz' },
  // Name says 6400MHz (and "VENGEANCE Black" with no "RGB"); column had speed=6000MHz and RGB=YES.
  '64GB Corsair VENGEANCE Black [DDR5, 6400MHz, 2x32GB]': { speed: '6400MHz', rgb: 'NO' },
  // Non-RGB "VENGEANCE Black" line (matches the 32GB/128GB non-RGB Vengeance Black entries
  // elsewhere in the sheet, both correctly tagged RGB=NO); this one had RGB=YES despite no "RGB"
  // in its own name.
  '96GB Corsair VENGEANCE Black [DDR5, 6400MHz, 2x48GB]': { rgb: 'NO' },
};

const FEATURE_KEYS = ['Memory Type', 'Memory Speed', 'RGB', 'Color', 'Memory Capacity'];
...
// "16GB Corsair VENGEANCE..." / "16GB Adata XPG Lancer..." / "128GB Samsung ECC..."
// XPG is Adata's own established vendor entity in this catalog (no separate "Adata" vendor
// exists), so "Adata XPG ..." kits are attributed to vendor "XPG".
function vendorForRam(name) {
  const rest = name.replace(/^\d+GB\s+/i, '');
  if (/^ADATA XPG/i.test(rest)) return 'XPG';
...
function parseMarkdown() {
  ...
  return rows.map((line) => {
    ...
    const [rawName, priceStr, memType, speed, rgb, color, capacity] = cols;
    ...
    return {
      ...
      memType,
      speed: correction.speed || speed,
      rgb: correction.rgb || rgb,
      color,
      capacity,
    };
  });
...
async function main() {
  ...
  console.log('Parsing RAM data sheet...');
  const rams = parseMarkdown();
  console.log(`Parsed ${rams.length} RAM rows.`);
  ...
  const unmatched = [];
  for (const r of rams) {
    if (imgDirSet.has(r.rawName)) matched.push(r);
    else unmatched.push(r.rawName);
  }
  console.log(`Matched ${matched.length}/${rams.length} RAM kits to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / type / speed / rgb / color / capacity):');
  for (const r of matched) {
    console.log(`  ${r.name} -> ${vendorForRam(r.name)} / AED ${r.price} / ${r.memType} / ${r.speed} / ${r.rgb} / ${r.color} / ${r.capacity}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((r) => vendorForRam(r.name)))];
  ...
  const toInsert = matched.filter((r) => !existingNames.has(normalizeName(r.name)));
  const skippedExisting = matched.length - toInsert.length;
  if (skippedExisting > 0) {
    console.log(`Skipping ${skippedExisting} products that already exist in the Ram category.`);
  }
  ...
  await runPool(toInsert, 6, async (r) => {
    const dirPath = path.join(IMAGES_DIR, r.rawName);
    let files;
    try {
      files = fs.readdirSync(dirPath).filter((f) => IMAGE_EXTS.has(path.extname(f).toLowerCase()));
    } catch (err) {
      failed++;
      failures.push({ name: r.name, error: `readdir failed: ${err.message}` });
      return;
    }
    if (files.length === 0) {
      failed++;
      failures.push({ name: r.name, error: 'no valid image files in folder' });
      return;
    }
    ...
    try {
      ...
      failed++;
      failures.push({ name: r.name, error: `S3 upload failed: ${err.message}` });
      return;
    }

    const vendorId = vendorIdByName.get(vendorForRam(r.name));
    ...
    try {
      ...
      const productResult = await client.query(
        `INSERT INTO products (name, category_id, price, image, stock, status, new_product)
         ...
         RETURNING id`,
        [r.name, categoryId, r.price, mainImageUrl, DEFAULT_STOCK]
      );
      ...
      const featureValues = [
        ['Memory Type', r.memType],
        ['Memory Speed', r.speed],
        ['RGB', r.rgb],
        ['Color', r.color],
        ['Memory Capacity', r.capacity],
      ];
      ...
      failed++;
      failures.push({ name: r.name, error: `DB insert failed: ${err.message}` });
    } finally {
      client.release();
    }
