...
const MD_FILE = path.join(__dirname, '..', 'Mouse.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'Mouse');
const CATEGORY_NAME = 'Mouse';
...
// No F- columns; Color and Wireless are parsed straight from each product's own name (no web
// lookups). Wireless is a real binary attribute so its absence from the name is treated as a
// meaningful "No" rather than skipped (e.g. "ASUS ROG Spatha X" doesn't say Wireless here even
// though the real product ships wireless — kept strictly to what this sheet's name says).
const FEATURE_KEYS = ['Color', 'Wireless'];
...
async function main() {
  ...
  console.log('Parsing Mouse data sheet...');
  const mice = parseMarkdown();
  console.log(`Parsed ${mice.length} mouse rows.`);
  ...
  console.log(`Matched ${matched.length}/${mice.length} mice to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / color / wireless):');
  for (const m of matched) {
    console.log(`  ${m.name} -> ${vendorForMouse(m.name)} / AED ${m.price} / ${m.color || '-'} / ${m.wireless}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((m) => vendorForMouse(m.name)))];
  ...
  if (skippedExisting > 0) {
    console.log(`Skipping ${skippedExisting} products that already exist in the Mouse category.`);
  }
  ...
  await runPool(toInsert, 6, async (m) => {
    ...
    const vendorId = vendorIdByName.get(vendorForMouse(m.name));
