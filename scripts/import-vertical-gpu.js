...
const MD_FILE = path.join(__dirname, '..', 'vertical-gpu.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'VerticalGPU');
// No dedicated category exists for this accessory (it's a mounting bracket, not a graphics
// card — putting it under "GPU" would pollute that category's VRAM/PSU-driven builder rules),
// so it goes into the existing generic "Other" category.
const CATEGORY_NAME = 'Other';
...
// No F- columns; Color and Compatible Case / PCIe Generation are parsed straight from each
// product's own name (no web lookups). NZXT's kit doesn't name a specific case or PCIe gen in
// its own name, so those two features are simply absent for those two rows.
const FEATURE_KEYS = ['Color', 'Compatible Case', 'PCIe Generation'];
...
function compatibleCaseFor(name) {
  const m = name.match(/^Lian Li (.+?) Vertical GPU Kit/i);
  return m ? `Lian Li ${m[1]}` : null;
}

function pcieGenerationFor(name) {
  const m = name.match(/PCI-?E\s*(\d(?:\.\d)?)/i);
  return m ? `PCIe ${m[1]}` : null;
}
...
function parseMarkdown() {
  ...
  return rows.map((line) => {
    ...
    return {
      ...
      compatibleCase: compatibleCaseFor(rawName),
      pcieGeneration: pcieGenerationFor(rawName),
    };
  });
}
...
async function main() {
  ...
  console.log('Parsing Vertical GPU Kit data sheet...');
  const kits = parseMarkdown();
  console.log(`Parsed ${kits.length} rows.`);
  ...
  console.log(`Matched ${matched.length}/${kits.length} kits to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / color / compatible case / pcie gen):');
  for (const k of matched) {
    console.log(`  ${k.name} -> ${vendorForKit(k.name)} / AED ${k.price} / ${k.color || '-'} / ${k.compatibleCase || '-'} / ${k.pcieGeneration || '-'}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((k) => vendorForKit(k.name)))];
  ...
  await runPool(toInsert, 6, async (k) => {
    ...
    const vendorId = vendorIdByName.get(vendorForKit(k.name));
    ...
    try {
      ...
      const featureValues = [
        ['Color', k.color],
        ['Compatible Case', k.compatibleCase],
        ['PCIe Generation', k.pcieGeneration],
  ...
  console.log('\n=== Vertical GPU Kit import complete ===');
