...
const MD_FILE = path.join(__dirname, '..', 'monitors.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'Monitors');
const CATEGORY_NAME = 'Monitor';
...
// All 13 rows' Display Size / Refresh Rate / Resolution were checked against each monitor's real
// published spec — every one matched exactly, no corrections needed here (unlike GPU/CPU/
// Motherboard/RAM).
const FEATURE_KEYS = ['Display Size', 'Refresh Rate', 'Resolution'];
...
function vendorForMonitor(name) {
...
function parseMarkdown() {
  ...
  return rows.map((line) => {
    ...
    const [rawName, priceStr, displaySize, refreshRate, resolution] = cols;
    return {
      rawName,
      name: rawName,
...
async function main() {
  ...
  console.log('Parsing Monitor data sheet...');
  const monitors = parseMarkdown();
  console.log(`Parsed ${monitors.length} monitor rows.`);
  ...
  for (const m of monitors) {
    if (imgDirSet.has(m.rawName)) matched.push(m);
    else unmatched.push(m.rawName);
  }
  console.log(`Matched ${matched.length}/${monitors.length} monitors to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / size / refresh / resolution):');
  for (const m of matched) {
    console.log(`  ${m.name} -> ${vendorForMonitor(m.name)} / AED ${m.price} / ${m.displaySize} / ${m.refreshRate} / ${m.resolution}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((m) => vendorForMonitor(m.name)))];
  ...
  if (skippedExisting > 0) {
    console.log(`Skipping ${skippedExisting} products that already exist in the Monitor category.`);
  }
  ...
  await runPool(toInsert, 6, async (m) => {
    const dirPath = path.join(IMAGES_DIR, m.rawName);
    let files;
    try {
    ...
    const vendorId = vendorIdByName.get(vendorForMonitor(m.name));
    ...
    try {
      ...
      const productResult = await client.query(
        `INSERT INTO products (name, category_id, price, image, stock, status, new_product)
         ...
         RETURNING id`,
        [m.name, categoryId, m.price, mainImageUrl, DEFAULT_STOCK]
      );
