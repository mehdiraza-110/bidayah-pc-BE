...
const MD_FILE = path.join(__dirname, '..', 'New-Product-Data-SSD.md');
const IMAGES_DIR = path.join(__dirname, '..', 'new-prod-pics', 'Hyper pc 1', 'SSD');
const CATEGORY_NAME = 'Storage';
...
// Row 2 is the only one written as "500gb ... [5000MBs, ...]" instead of the "500GB ...
// [5000MB/s, ...]" format every other row uses — cosmetic only, fixed for consistency.
const CORRECTIONS = {
  '500gb Kingston NV3 [5000MBs, Gen4]': { nameOverride: '500GB Kingston NV3 [5000MB/s, Gen4]' },
};

// Like the Coolers sheet, this one has no F- columns — but unlike coolers, every spec that
// matters (capacity, read speed, PCIe generation) is already encoded in each product's own
// name, so no research guesswork is needed for those. Form Factor and Interface are constant
// across all 10 rows (verified against Kingston/Samsung/Corsair's own spec pages: all four
// models — NV3, 990 PRO, 9100 PRO, MP600 PRO — are M.2 2280 NVMe drives).
const FORM_FACTOR = 'M.2 2280';
const INTERFACE = 'NVMe';

const FEATURE_KEYS = ['Capacity', 'Read Speed', 'Interface', 'PCIe Generation', 'Form Factor'];
...
// The image folders were created with "/" stripped or replaced with "_" inconsistently
// (e.g. "6000MB/s" -> "6000MBs" in one folder, "14700MB/s" -> "14700MB_s" in another). Match
// by dropping both "/" and "_" from each side rather than requiring an exact string match.
...
function vendorForSsd(name) {
  const rest = name.replace(/^\d+\s*(GB|TB)\s+/i, '');
...
function parseMarkdown() {
  ...
  return rows.map((line) => {
    ...
    const capMatch = rawName.match(/^(\d+)\s*(GB|TB)/i);
    const specMatch = rawName.match(/\[(\d+)\s*MB[/_]?s,\s*Gen(\d+)\]/i);
    if (!capMatch || !specMatch) {
      throw new Error(`Could not parse capacity/speed/gen from "${rawName}"`);
    }

    return {
      rawName, // used to locate the image folder
      name: correction.nameOverride || rawName,
      price: parsePrice(priceStr),
      capacity: `${capMatch[1]}${capMatch[2].toUpperCase()}`,
      readSpeed: `${specMatch[1]}MB/s`,
      pcieGeneration: `Gen${specMatch[2]}`,
    };
  });
}
...
async function main() {
  ...
  console.log('Parsing SSD data sheet...');
  const ssds = parseMarkdown();
  console.log(`Parsed ${ssds.length} SSD rows.`);

  const imgDirs = fs.readdirSync(IMAGES_DIR, { withFileTypes: true })
    .filter((e) => e.isDirectory())
    .map((e) => e.name);
  const dirByNorm = new Map();
  for (const d of imgDirs) dirByNorm.set(normalizeForFolderMatch(d), d);

  const matched = [];
  const unmatched = [];
  for (const s of ssds) {
    const dir = dirByNorm.get(normalizeForFolderMatch(s.rawName));
    if (dir) matched.push({ ...s, dir });
    else unmatched.push(s.rawName);
  }
  console.log(`Matched ${matched.length}/${ssds.length} SSDs to image folders.`);
  if (unmatched.length) {
    console.log('Unmatched (no image folder found):');
    unmatched.forEach((n) => console.log(`  - ${n}`));
  }

  console.log('\nPreview (name -> vendor / price / capacity / speed / gen):');
  for (const s of matched) {
    console.log(`  ${s.name} -> ${vendorForSsd(s.name)} / AED ${s.price} / ${s.capacity} / ${s.readSpeed} / ${s.pcieGeneration}`);
  }
  ...
  const neededVendorNames = [...new Set(matched.map((s) => vendorForSsd(s.name)))];
  ...
  const toInsert = matched.filter((s) => !existingNames.has(normalizeName(s.name)));
  const skippedExisting = matched.length - toInsert.length;
  if (skippedExisting > 0) {
    console.log(`Skipping ${skippedExisting} products that already exist in the Storage category.`);
  }
  ...
  await runPool(toInsert, 6, async (s) => {
    const dirPath = path.join(IMAGES_DIR, s.dir);
    let files;
    try {
      files = fs.readdirSync(dirPath).filter((f) => IMAGE_EXTS.has(path.extname(f).toLowerCase()));
    } catch (err) {
      failed++;
      failures.push({ name: s.name, error: `readdir failed: ${err.message}` });
      return;
    }
    if (files.length === 0) {
      failed++;
      failures.push({ name: s.name, error: 'no valid image files in folder' });
      return;
    }
    ...
    try {
      ...
      failed++;
      failures.push({ name: s.name, error: `S3 upload failed: ${err.message}` });
      return;
    }

    const vendorId = vendorIdByName.get(vendorForSsd(s.name));
    ...
    try {
      ...
      const productResult = await client.query(
        `INSERT INTO products (name, category_id, price, image, stock, status, new_product)
         ...
         RETURNING id`,
        [s.name, categoryId, s.price, mainImageUrl, DEFAULT_STOCK]
      );
      ...
      const featureValues = [
        ['Capacity', s.capacity],
        ['Read Speed', s.readSpeed],
        ['Interface', INTERFACE],
        ['PCIe Generation', s.pcieGeneration],
      ...
      failed++;
      failures.push({ name: s.name, error: `DB insert failed: ${err.message}` });
    } finally {
      client.release();
    }
